...
## Current Milestone
Template Builder v1.2 (T071–T076) complete. Template Builder v1.1 (T063–T070) complete. Template Builder v1 (T056–T062) complete.
- **Template Builder v1.2 (T071–T076)**: Visual effectiveness pass. Logo scale presets widened (small=6mm, large=22mm) for strong visible difference. Trading name font scales with logo scale in both Preview and PDF. Legal line de-emphasized (smaller, subtler). Density presets produce material visible changes: comfortable has generous padding/spacing, compact has tight padding, small drawings, compressed spec rows. `TemplateDensity` extended with `itemCardPadMm` and `itemGapMm`. Preview now reflects: section gaps (spacing preset), item card padding/header padding/spec row heights/drawing heights (density), photo sizes (photo size preset), inter-item gaps (density). Photo size control wired into Preview (was PDF-only). Typography and spacing labels clarified to indicate scope (PDF-only vs both). All builder controls are now either visually effective or clearly labeled. No dead controls remain. Architecture guards preserved — no schema/API/lifecycle changes.
