...
**Invoice Lines Foundation (Stage 1+2)**: `invoice_lines` table provides row-level line-item storage (id, invoiceId, sortOrder, lineType, description, quantity, unitAmount, lineAmountExclGst, variationId, sourceContext). Dual-write on `POST /api/invoices` creates a matching single line. `GET /api/invoices/:id/lines` reads lines. Idempotent backfill migration (`server/invoice-lines-backfill.ts`) populates lines for pre-existing invoices on startup. Header-level `amountExclGst` remains the source of truth for allocation and Xero push through Stage 4.
