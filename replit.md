...
## Current Milestone
Division Settings UX Cleanup (T101–T106) complete. Saved Config Calibration (T096–T100) complete. Default Tuning Pass (T090–T095) complete. Template Builder v1.3 (T083–T089) complete. Company Master Template v2 (T077–T082) complete.
- **Division Settings UX Cleanup (T101–T106)**: Division page reorganized into two clearly separated sections: (1) Quote Presentation — inherited company template info card (lock icon, live templateKey, helper text), division branding, layout/theme/content overrides, spec display defaults; (2) On-Site Estimate Defaults — job type presets (renovation/new build). Locked template field replaced from disabled input to informational display showing `form.templateKey`. SiteVisitPresetsCard renamed to "Job Type Presets" with workflow-context helper text. Section headers use uppercase + icons + descriptive subtext. Visual separator between the two sections. No architecture, schema, or behavior changes.
