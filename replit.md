...
- **Customer-Facing Quote Summary**: Shows Items Subtotal, Installation (sell), Delivery (sell or "Supply Only" note), Subtotal excl. GST, GST line, Total incl. GST. No cost data shown. Items table includes Weight column (always visible). Stats cards: Total Items, Total m², Total Weight (always visible) + Avg $/m² (togglable). Unified pricing toggle (`showPricingOnQuote`, default false) controls $/m² column, Price column, and Avg $/m² card together — hidden by default for customer-facing view. Weight data cached as `cachedWeightKg` on QuoteItem config when saved from quote builder.
