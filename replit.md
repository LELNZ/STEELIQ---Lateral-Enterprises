...
**UI/UX**: Responsive tables, polished spec row formatting in PDFs, project dashboard guidance, customer relink safeguards, and an enterprise app shell with workflow-domain grouped navigation. Accepted quote workflow progress panel guides users through project, job, and invoice creation. Estimate list implements a three-signal commercial control surface: (1) **Estimate Status** column (primary, solid badge — Active/Converted/Archived), (2) **Quote** column (secondary, outline badge — Draft/Review/Sent/Accepted/Declined/Archived with clickable quote number link), (3) Revision signal (tertiary, inline text — "Rev N"). Estimate status is derived client-side: `archivedAt` → Archived, linked quote with status "accepted" → Converted, otherwise → Active. Server `/api/jobs` enriches each job with `linkedQuotes` metadata via batch SQL join. Quote signal rendering is shared via `QuoteSignalCell` component across active and archived tabs.
