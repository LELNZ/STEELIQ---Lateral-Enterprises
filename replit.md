...
**LL Estimate Persistence Layer (Phase 3C)**: LL now has a proper estimate-first workflow with persistent records. The `laser_estimates` table stores estimate records with `estimateNumber` (format `LE-XXXX-LL`), `customerName`, `projectAddress`, `status` (draft/ready/converted/archived), `itemsJson` (JSONB array of LaserQuoteItem[]), and optional `customerId`/`contactId`. The `quotes` table has `sourceLaserEstimateId` for provenance linkage from estimates to quotes. When a quote is generated from an estimate, the estimate status auto-transitions to `converted`. A dedicated `/laser-estimates` list page shows saved LL estimates with New/Open/Delete actions (delete blocked for converted estimates). The laser builder supports `estimateMode` for create/save/update estimate flows plus a "Generate Quote" action with source linkage. All `/api/laser-estimates` routes enforce LL division access control. Source estimate validation on quote creation checks existence before linking. Sidebar LL link points to `/laser-estimates`; routes: `/laser-estimates`, `/laser-estimate/new`, `/laser-estimate/:id`.
