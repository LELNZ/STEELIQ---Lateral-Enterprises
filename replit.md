...
## Current Milestone
Template Builder v1.1 (T063–T070) complete. Template Builder v1 (T056–T062) complete. Master template visual refinement (T049–T055) complete. Template visual application (T043–T047) complete. Division template threading (T038–T042) complete.
- **Template Builder v1.1 (T063–T070)**: Extended template architecture with `TemplateHeader` (logoScale, showTradingName, legalLinePlacement), `TemplateDensity` (drawingMaxH, specRowH, itemHeaderH, photoRowH), and `DocumentMode` ("standard"|"tender"). Added `LOGO_SCALE_PRESETS` (small/standard/large) and `DENSITY_PRESETS` (comfortable/standard/compact). `CompanyTemplateConfig` extended with `logoScale`, `showTradingName`, `densityPreset`, `documentMode`. All wired into PDF engine (renderHeader uses logo scale + showTradingName, renderScheduleItem uses density values, renderQuotationTitle uses documentMode) and preview (HeaderSection, ScheduleItemCard, quotation title). Settings Template tab has new "Header & Branding" card (logo size, trading name toggle, document type) and density control in Schedule card. Server Zod validation updated for all new fields.
