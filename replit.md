...
- **Handle Categories**: 7 category-specific handle types with collapsible UI in Library page. Allocation uses `categoryMatch` field for rename-proof frame type lookup (matches by category array, not value string). Per-entry multi-select allocation: each handle entry can be allocated to multiple frame types via checkboxes in the edit dialog. Stored as `data.allocations` array; falls back to category-level default when not set
- **Liner Types**: Per-entry multi-select frame type allocation via checkboxes in edit dialog. Stored as `data.allocations` array; falls back to "All Frame Types" when not set
- **Wanz Bar (Sill Support)**: 6 WANZ products (19mm–55mm) in Library with kg/m, USD/kg, NZD/lin.m pricing. Auto-seeded. Source selection: NZ Local or Direct Supplier. Auto-enable hint at width ≥ 600mm. Pricing: NZ Local = priceNzdPerLinM × width; Direct = pricePerKgUsd × kgPerMetre × width × rate. Per-entry multi-select frame type allocation via checkboxes in edit dialog. Stored as `data.allocations` array; falls back to "All Windows" when not set. Included in net cost and exec summary
